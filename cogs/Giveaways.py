...
class Giveaways(commands.Cog):
    ...
    # Embeds
    # --------------------------------------------------------
    def _embed(self, title: str, description: str, colour: discord.Color) -> discord.Embed:
    ...
    def _winners_embed(
        ...
        return discord.Embed(title=title, description=desc, color=self.info_colour)

    def _dm_winner_embed(
        self, guild_name: str, prize: str, host_id: Optional[int]
    ) -> discord.Embed:
        contact_line = (
    ...
    def _dm_host_embed(
        self, giveaway_id: int, prize: str, winners: Sequence[int], is_reroll: bool
    ) -> discord.Embed:
        if winners:
            mentions = " ".join(f"<@{uid}>" for uid in winners)
        ...
        return discord.Embed(
            title="Giveaway Result", description=desc, color=self.info_colour
        )
    ...
    def _build_giveaway_embed(
        ...
        colour = (
            ...
            if status == "running"
                ...
                self.error_colour if status == "cancelled" else self.info_colour
            )
        )
        embed = discord.Embed(
            title="🎁 Giveaway",
    ...
    async def giveaway_end(self, interaction: discord.Interaction, giveaway_id: int):
        ...
        if row["status"] != "running":
            await interaction.followup.send(
                embed=self._embed(
                    "Not running", "That giveaway is not running.", self.error_colour
                ),
                ephemeral=True,
            )
            return
