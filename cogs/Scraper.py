...
class Scrape(commands.Cog):
    def __init__(self, bot):
        ...
        live_cfg = self.config.get("features", {}).get("live_events", {})
        # The URL containing the <section id="live"> structure.
        self.LIVE_PAGE_URL = live_cfg.get(
    ...
    @app_commands.command(
        name="scrape",
        description="Checks the configured website for new shows and updates #live-shows and server events.",
    )
    async def scrape(self, interaction: discord.Interaction):
        ...
        try:
            ...
            error_embed = discord.Embed(
                title="Scrape Failed",
                description=(
                    "I could not complete the scrape. Please check the configured website and try again shortly."
                ),
                color=self.error_colour,
            )
            try:
    ...
    def run_scraper(self) -> List[Tuple[str, str, str, Optional[str]]]:
        """
        Scrape the configured homepage for the <section id="live"> structure and extract
        ...
        """
        logging.info("Running scraper by parsing HTML from configured live section...")
        audit_log(
    ...
    async def check_forum_threads(self, guild, interaction, new_entries):
        ...
        liveshows_channel_id = self.channels.get("liveshows_forum_id")
        if not liveshows_channel_id:
            logging.error("Missing 'channels.liveshows_forum_id' in config.")
            await interaction.followup.send(
                embed=discord.Embed(
                    title="Forum Channel Not Configured",
                    description=(
                        "Set `channels.liveshows_forum_id` in config.yaml so I know which forum to update."
                    ),
                    color=self.error_colour,
                )
            )
        ...
        if liveshows_channel is None:
            ...
            error_embed = discord.Embed(
                title="Forum Channel Missing",
                description=(
                    "I could not find that forum channel. Update the ID in config.yaml or recreate the channel."
                ),
                color=self.error_colour,
            )
            await interaction.followup.send(embed=error_embed)
            ...
            if not exists:
                try:
                    if venue and location:
                        content_base = (
                            f"Live show at {venue.title()}, {location.title()}"
                        )
                    elif venue:
                        content_base = f"Live show at {venue.title()}"
                    elif location:
                        content_base = f"Live show in {location.title()}"
                    else:
                        content_base = "Live show update"
                    content = (
                        f"{content_base}\nTickets: {tickets_url}"
                        if tickets_url
                        else content_base
                    ...
                    error_embed = discord.Embed(
                        ...
                        description=(
                            f"I cannot create threads in {liveshows_channel.mention}. "
                            "Give me Create Public Threads permission or adjust the forum permissions."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=error_embed)
                    ...
                    error_embed = discord.Embed(
                        ...
                        description=(
                            f"I could not create the thread '{thread_title}'. Please try again shortly."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=error_embed)
            ...
            if not exists:
                ...
                if venue or location:
                    description_lines.append(
                        f"Live show at {venue.title() if venue else ''}{', ' if venue and location else ''}{location.title() if location else ''}".strip(
                            ", "
                        )
                    )
                if tickets_url:
                ...
                description = (
                    "\n".join(description_lines) if description_lines else "Live show update"
                )
                ...
                try:
                    ...
                    error_embed = discord.Embed(
                        title="Missing Permission",
                        description=(
                            "I need the Manage Events permission to schedule server events. "
                    ...
                    error_embed = discord.Embed(
                        ...
                        description=(
                            f"I could not create the event '{event_name}'. Please try again shortly."
                        ),
                        color=self.error_colour,
                    )
                    await interaction.followup.send(embed=error_embed)
